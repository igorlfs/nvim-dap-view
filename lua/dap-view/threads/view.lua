...
---@class dapview.Label
---@field labels {label_part: string, hl?: string | true}
---@field id string
...
M.show = function()
    ...
    if util.is_buf_valid(state.bufnr) and util.is_win_valid(state.winnr) then
        ...
        for k, _ in pairs(state.file_paths_by_frame_id) do
            state.file_paths_by_frame_id[k] = nil
        end
        for k, _ in pairs(state.file_location_by_frame_id) do
            state.file_location_by_frame_id[k] = nil
        end
                ...
                local frames = vim.iter(valid_frames):fold(
                    ...
                    function(acc, f)
                        ...
                        if show_frame then
                            ...
                            state.file_paths_by_frame_id[f.id] = relative_path
                            state.file_location_by_frame_id[f.id] = f.line

                            table.insert(acc, {
                                labels = {
                                    { label_part = relative_path, hl = "FileName" },
                                    { label_part = tostring(f.line), hl = "LineNumber" },
                                    { label_part = f.name },
                                },
                                id = f.id,
                            })
                    ...
                    :filter(
                        ...
                        function(f)
                            ...
                            for _, l in ipairs(f.labels) do
                                ---@cast l {label_part: string}
                                label = label .. l.label_part
                            end
                ...
                for i, f in pairs(filtered_frames) do
                    ...
                    if session.current_frame and f.id == session.current_frame.id then
                        ...
                        for _, p in ipairs(f.labels) do
                            ---@cast p {label_part: string, hl?: string | true}

                            if p.hl and type(p.hl) == "string" then
                                local hl_end = hl_init + #p.label_part
                            ...
                            hl_init = hl_init + #p.label_part + 1
                        end
                    end
