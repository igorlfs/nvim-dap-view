...
local highlight_file_name_and_line_number = function(row, len_path, len_lnum)
    if state.bufnr then
        ...
        hl.hl_range("NvimDapViewFileName", { row, 0 }, { row, len_path })
        hl.hl_range("NvimDapViewLineNumber", { row, lnum_start }, { row, lnum_end })
        hl.hl_range("NvimDapViewSeparator", { row, lnum_start - 1 }, { row, lnum_start })
        hl.hl_range("NvimDapViewSeparator", { row, lnum_end }, { row, lnum_end + 1 })
    end
end
